...
func validateCommon(orderID string, amount float64, branchID string) error {
	switch {
	case orderID == "":
		return invalid("OrderID", "required")
	case len(orderID) > maxOrderIDLen:
		return invalid("OrderID", "at most 128 characters")
	case amount != 0 && amount < minAmount:
		return invalid("Amount", "minimum 0.01")
	case amount != 0 && !hasAtMostTwoDecimals(amount):
		return invalid("Amount", "at most 2 decimal places")
	case len(branchID) > maxBranchIDLen:
		return invalid("BranchID", "at most 64 characters")
	}
	return nil
}
...
func (in ProcessApplePayInput) validate() error {
	...
	if in.Token.PaymentData.Data == "" || in.Token.TransactionIdentifier == "" {
		return invalid("Token", "must be the PKPaymentToken as received from Apple Pay")
	}
	return nil
}
...
func (in ProcessGooglePayInput) validate() error {
	...
	return invalid("CurrencyCode", "must be MNT, USD, EUR or JPY")
}
