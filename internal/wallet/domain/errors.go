...
// aggregate's *Input type documents its own Validate(); it is a lowerCamelCase parameter
// name (e.g. "paymentID") when the check is on a bare scalar argument that has no struct
// field of its own. Match on the sentinel with errors.Is, not on this string.
